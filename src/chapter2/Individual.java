...
public class Individual {
	...
	/**
	 * Individual类代表一个候选解，主要负责存储和操作一条染色体。 请注意Individual类有两个构造方法：
	 * 一个构造方法接收一个整数(代表染色体的长度)，在初始化对象时将创建一条随机的染色体 一个构造方法接受一个整数数组，用它作为染色体
	 * 
	 * @author cloud
	 *
	 */
	...
	/**
	 * Initializes individual with specific chromosome 用特定染色体初始化个体
	 * 
	 * @param chromosome
	 *            染色体 The chromosome to give individual 赋给个体的染色体
	 */
	...
	/**
	 * Initializes random individual. 初始化随机个体
	 * 
	 ...
	 * 这个构造函数假设染色体完全由0和1组成，这可能并非总是如此， 因此请务必根据需要进行修改。 这个构造函数还假设“随机”染色体意味着简单地选择随机0和1，
	 * 这也可能不是这种情况（例如，在旅行商问题中，这将是无效的解决方案）。
	 * 
	 * @param chromosomeLength
	 *            染色体长度 The length of the individuals chromosome 染色体个体长度
	 */
	...
	/**
	 * Gets individual's chromosome 返回染色体个体
	 * 
	...
	/**
	 * Gets individual's chromosome length 获取染色体长度
	 * 
	...
	/**
	 * Set gene at offset offset用于传入位于染色体上的基因位置索引 gene代表基因位上的值
	 * 
	 * @param gene
	 * @param offset
	 * @return gene
	...
	/**
	 * Get gene at offset 返回特定offset位置处的基因
	 * 
	 * @param offset
	 * @return gene
	 */
	...
	/**
	 * Store individual's fitness 存储个体的适应度值
	 * 
	 * @param fitness
	...
	/**
	 * Gets individual's fitness 获得个体适应度值
	 * 
	...
	/**
	 * Display the chromosome as a string. 将染色体显示为字符串
	 * 
